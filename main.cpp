	...
	for (i = 1; i < argc; i++) {
		...
		if (!strcmp(argv[i], option) || !strcmp(argv[i], shortOption)) {
			if (!strcmp(option, "--collector") || !strcmp(shortOption, "-c")) {
				...
				if (!strcmp(argv[i + 1], "markSweepRB"))
					return (int)markSweepRB;
				return -1;
...
int main(int argc, char *argv[]) {
	if(argc < 2) {
		fprintf(stderr, "Usage: TraceFileSimulator traceFile [OPTIONS]\n" \
						...
						"  --collector x, -c x       uses x as the garbage collector (valid: markSweep, markSweepRB, traversal, default: traversal)\n" \
						"  --traversal x, -t x       uses x as the traversal algorithm (valid: breadthFirst depthFirst hotness, default: breadthFirst)\n" \
						"  --allocator x, -a x       uses x as the allocator (valid: real, basic, regionBased, nextFit default: nextFit)\n" \
